...
def main():
    ...
    for epoch in range(0, epoch_num):
        ...
        for i, data in enumerate(salobj_dataloader):
            ...
            Pan, labels, Mul = data['image'], data['label'], data['LBP']
            Pan = Pan.type(torch.FloatTensor)
            Mul = Mul.type(torch.FloatTensor)
            labels = labels.type(torch.FloatTensor)
            # wrap them in Variable
            Pan_v, labels_v, Mul_v = Variable(Pan.cuda(), requires_grad=False), \
            ...
            d0, d1, d2, d3, d4, d5, d6, cx0= net(Pan_v, Mul_v)
